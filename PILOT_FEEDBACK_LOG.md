...
| 2026-08-04 | Institution | 📱 Real device (screenshot) + 🐛 code confirmed | Institution registration either succeeds, or fails with a clear reason and a way to back out (sign out, retry with a different account) | Submitting failed with an unexplained "Acesso não autorizado," and `RegisterScreen` has zero exit — no cancel, no back, no sign-out. Force-quitting/reopening the app always lands back on the same screen, since `RootNavigator` routes any signed-in user with no Institution record straight here with nothing else reachable. Root cause of the error itself, confirmed against live Sheets data: Role is fixed at an account's first-ever sign-up and never auto-updates; signing into the Institution app with an email that already has a Donor account passes auth but fails `requireRole('Institution')` on submit. | Single Screen (missing exit) + Shared Service (`requireRole`'s uninformative message, fires everywhere this gate is used) | Added a "Não é a sua conta? Sair" sign-out option to `RegisterScreen` (mirrors `VerificationStatusScreen`, the very next screen, which already had one); `requireRole`'s 403 now names the actual role mismatch instead of a generic message. **Follow-up (c4e2f1b):** the mismatch is now caught immediately at sign-in (`resolveSession`), before the user ever reaches the registration form — message names the existing role and explicitly says to use a different email, not just that something's wrong | Fixed (code) | ✅ Verified on real device (Wafina Instituição build 5, OnePlus 7 Pro) — sign-in now blocks a role-mismatched account immediately with a clear message; RegisterScreen's sign-out option confirmed present |

**Immediate workaround for anyone already stuck on this screen, before the next APK exists:** Android
Settings → Apps → Wafina Instituição → Storage → Clear Data. This wipes the local Firebase session
and drops you back to Sign In — no data loss beyond that local session, since nothing was ever saved
server-side (the registration never succeeded). Sign in with the *correct* account for testing
Institution (not an email already used as a Donor).
