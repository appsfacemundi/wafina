...
| Android — Institution App first production build (`wafina-institution`, new EAS project) | 2026-08-03 | **PASS.** Product decision recorded: institution registration stays open (Admin review/approval), invitation codes explicitly out of scope for V1. Build infra added mirroring Donor's fixed config from day one — `apps/mobile-institution/eas.json` (new, with `production.env` block), `eas-build-post-install` hook, EAS project linked via `eas init --force` (`0a4405fe-c166-4226-9b7d-f11abd9ed01c`). First build, Build ID `cab3ad85-c6b4-491b-ad25-21385325bfa7` (https://expo.dev/accounts/zuinder/projects/wafina-institution/builds/cab3ad85-c6b4-491b-ad25-21385325bfa7), `BUILD SUCCESSFUL in 12m 41s`, `app-release.aab` (52.1 MB), status `FINISHED`, commit `7d9f497`. Verified identically to Donor: EAS CLI confirmed all 5 env vars loaded; byte-search of the downloaded AAB's Hermes bundle found the real Firebase key/project ID/App ID and real API URL each exactly once, `localhost:4000` zero times; `eas-build-post-install` rebuilt `packages/shared` cleanly; `EAGER_BUNDLE` succeeded (959 modules); AAB download link reachable; CI green (`30800560120`). Also added a support-contact message (`geral@zuinder.com`) to `VerificationStatusScreen`'s rejected-institution branch — explicitly not a self-service resubmission flow, per product decision. **Not verified: on-device runtime flows** — same environmental limitation as Donor (no Android SDK/emulator/Play Console access here). | `7d9f497` |
