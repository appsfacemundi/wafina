...
| 2026-08-02 | Stakeholder-reported "critical blocking bug": a donor's new donation never appeared in the Institution app's Available Donations | **Investigated and reproduced against live production — the pipeline itself is not broken.** `listAvailableDonations` deliberately scopes by `Institutions.Country_ID` (Phase 3A Module 1, intentional multi-country design), confirmed working end-to-end (create → matches → visible → claim → schedule → collect → deliver, all verified live via direct API calls with real test accounts). Also confirmed the "incomplete profile" edge case is already safely blocked server-side (`400 "Complete o seu perfil (incluindo o país) antes de doar"`). Most likely explanation for what the stakeholder saw: the specific donor/institution accounts used were registered under different countries — correct behavior, not a defect. **Real gap found and fixed regardless:** the Available Donations empty state gave no indication that the list is country-scoped, making a legitimate mismatch indistinguishable from a broken pipeline. Awaiting stakeholder confirmation of which two accounts/countries were used to close this out definitively. | `apps/institution/src/app/donations/available/page.tsx` (empty-state message now names the institution's own country and explains the scoping, instead of a generic "no donations yet") | pending |
